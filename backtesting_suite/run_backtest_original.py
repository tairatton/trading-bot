"""
Original Bot Backtest (Dedicated)
- Defaults: 3 Pairs, Risk 0.70%, Aggressive Tiers
- Output: backtest_results_original
"""
import argparse
import sys
import os
...
def main():
    ...
    # Original Bot Defaults (0.70% Risk)
    parser.add_argument("--pairs", nargs="+", default=["EURUSDm", "USDCADm", "USDCHFm"])
    parser.add_argument("--days", type=int, default=365)
    parser.add_argument("--risk", type=float, default=0.0070)
    parser.add_argument("--output", type=str, default="backtest_results_original")
    args = parser.parse_args()
    
    print("=" * 60)
    print("ORIGINAL BOT BACKTEST (Aggressive Mode)")
    print("=" * 60)
    print(f"Pairs: {', '.join(args.pairs)}")
    print(f"Risk per pair: {args.risk * 100:.2f}%")
    print()
    
    strategy = get_strategy_module("original")
    ...
    # Original Bot 5-Tier Config (Aggressive)
    risk_tiers = [
        (5.0, 0.8),
        (10.0, 0.6),
        (15.0, 0.4),
    ...
    daily_loss = 10.0 # Aggressive mode safety net
